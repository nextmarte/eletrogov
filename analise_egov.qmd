...
d_idade_min <- descritivas %>% slice_min(idade_media, n = 1)
d_idade_max <- descritivas %>% slice_max(idade_media, n = 1)
d_pea_min   <- descritivas %>% slice_min(prop_pea, n = 1)
d_pea_max   <- descritivas %>% slice_max(prop_pea, n = 1)
d_h2_min    <- descritivas %>% slice_min(prop_h2, n = 1)
d_h2_max    <- descritivas %>% slice_max(prop_h2, n = 1)
d_n_min     <- descritivas %>% slice_min(n, n = 1)
d_n_max     <- descritivas %>% slice_max(n, n = 1)
...
**Envelhecimento progressivo da base de usuários de internet.** A idade média subiu de `r round(d_idade_min$idade_media, 1)` anos (`r d_idade_min$ano`) para `r round(d_idade_max$idade_media, 1)` anos (`r d_idade_max$ano`). Em 2019 — ano-base do artigo original — a média era `r round(d_2019$idade_media, 1)` anos. Esse envelhecimento reflete a universalização do acesso à internet entre faixas etárias mais velhas. Como a idade tem efeito negativo sobre o uso de e-gov, a estabilidade do coeficiente de idade ao longo dos anos (ver @sec-longitudinal) torna-se ainda mais relevante: o barreira etária persiste mesmo com a popularização.

**Queda da proporção de PEA na amostra.** A parcela de economicamente ativos caiu de `r fmt_pct(d_pea_max$prop_pea)` (`r d_pea_max$ano`) para `r fmt_pct(d_pea_min$prop_pea)` (`r d_pea_min$ano`). Isso indica que a internet se expandiu para aposentados, estudantes e pessoas fora do mercado de trabalho. Vargas et al. (2021) encontraram a PEA como preditor significativo em 2019 (quando `r fmt_pct(d_2019$prop_pea)` eram PEA) — a queda dessa proporção ajuda a explicar por que o coeficiente da PEA se enfraqueceu nos anos recentes.

**E-commerce relativamente estável.** A proporção de compradores online oscilou entre `r fmt_pct(d_h2_min$prop_h2)` e `r fmt_pct(d_h2_max$prop_h2)` sem tendência clara. Isso sugere que o e-commerce, diferentemente do acesso à internet, não se universalizou na mesma velocidade — e explica por que ele permanece como o preditor mais discriminante: quem compra online continua sendo um subgrupo com maior letramento digital.

**Crescimento amostral.** O N após filtros cresceu de `r fmt_n(d_n_min$n)` (`r d_n_min$ano`) para `r fmt_n(d_n_max$n)` (`r d_n_max$ano`), refletindo a ampliação da base de respondentes que usam internet (filtro C1). O N reduzido de 2015 deve-se à ausência de `C5_Dispositivos`, que exclui muitas observações por *missing* naquele ano.
...
::: {.callout-tip}
## Correção identificada durante a replicação
A versão inicial deste documento usava o filtro `C1 != 0`, que preserva respostas 97 ("Não sabe") e 98 ("Não respondeu") além de 1 ("Sim"). O artigo original usa `C1 == 1` — mantendo apenas quem **afirmou** ter usado internet. Essa diferença de 14 observações no passo 3 propagava-se como 3 observações extras no N final. A correção para `C1 == 1` elimina a discrepância.
:::
...
**PEA (condição de atividade)** — Efeito positivo moderado (**`r fmt_c(pea_range[1])` a `r fmt_c(pea_range[2])`**), com leve tendência de **queda** ao longo dos anos. Em 2015, ser economicamente ativo aumentava mais a chance de usar e-gov. Em 2024–2025, esse efeito é menor, possivelmente porque serviços sociais (Auxílio Brasil, Bolsa Família) passaram a ser acessados digitalmente por pessoas fora da PEA.
...
- A **PEA** perdeu força preditiva ao longo dos anos (de `r fmt_c(max(coef_pea$estimate))` em `r coef_pea$ano[which.max(coef_pea$estimate)]` para `r fmt_c(min(coef_pea$estimate))` em `r coef_pea$ano[which.min(coef_pea$estimate)]`), possivelmente refletindo a expansão de serviços de e-gov voltados a populações economicamente inativas (programas sociais digitalizados)
